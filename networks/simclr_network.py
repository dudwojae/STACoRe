...
# MLP class for SimCLR projector
class MLPHead(nn.Module):
    def __init__(self, in_features, projection_size):
...
class SimCLR_DQN(nn.Module):
    def __init__(self, args, action_space):
        super(SimCLR_DQN, self).__init__()
        self.args = args
    ...
    def forward(self, x, log=False):
        ...
        if isinstance(self.projector, nn.Module):
            proj_z = self.projector(x)

        else:
            proj_z = torch.matmul(x, self.W_h) + self.b_h  # Contrastive head
